...
2. **Acquisition (Resumes/Crashes):** If row already exists:
   ```sql
   UPDATE step_results 
   ...
   ```
3. **Heartbeat:** Update `lease_heartbeat_at` every 30s.
...
## 13. Changes from v1.10 (adversarial review fixes)

| # | Section | Change | Reason |
|---|---|---|---|
| 58 | §6.3 | Update Lease logic with SQLite-valid syntax and New-Step Initiation | `NOW()` is invalid SQLite; spec lacked definition for claiming previously unlogged steps |
| 59 | §3 | Temper "Absolute Determinism" to "Prompt & Parameter Determinism" | Avoid overstating guarantees regarding external binaries/assets not hashed in v1 |
