...
4. **Heartbeat:** Active workers update `lease_heartbeat_at` every 30s.

### 6.4 YouTube upload — Manual Reconciliation Gate
(Identical to v1.10)

### 6.5 Remote Side-Effect Protection Matrix (LLM, TTS)
Billing and logic protection is tiered based on provider capabilities:

| Tier | Protection | Providers | Risk |
|---|---|---|---|
| **Tier 1 (Strong)** | Real Idempotency Keys (`X-Idempotency-Key`) | OpenAI, OpenRouter | **Zero** billing risk on crash-recovery. |
| **Tier 2 (Artifact-First)** | Artifact existence check before call | Anthropic, Gemini, TTS vendors | **Low** risk: crash *during* API response can cause double billing. |
| **Manual (Safe)** | User-generated scripts / local files | Gemini manual mode, Local TTS | **Zero** billing risk. |

The orchestrator MUST perform an **Artifact-First Recovery** check (disk existence) for all Tiers before making a network call.
...
## 13. Changes from v1.13 (adversarial review fixes)

| # | Section | Change | Reason |
|---|---|---|---|
| 62 | §6.3 | Define explicit Lease Expiry (120s) and recovery predicate | Prevent steps from becoming permanently stuck after a worker crash |
| 63 | §6.5 | Categorize "Provider Reliability Matrix" (Tier 1 vs Tier 2) | Align spec promises with real-world provider limitations; acknowledge small billing risk for non-idempotent APIs |
