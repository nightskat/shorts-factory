...
def test_run_success(memory_db, tmp_path):
    ...
    with patch(
        "shorts.nodes.idea_gen.get_llm_provider", return_value=mock_llm
    ) as mock_get_llm:
        with patch(
            "shorts.nodes.idea_gen.inject_voice_into_system_prompt",
            return_value="Injected Prompt",
        ) as mock_inject:
            result = run("job123", execution_context, memory_db, services)

    assert result.status == "done"
    assert result.output_path == os.path.join(
        str(tmp_path), "data", "scripts", "job123_script.txt"
    )
    assert result.output_checksum is not None

    mock_get_llm.assert_called_once_with("dummy", env)
    mock_inject.assert_called_once_with(
        "You are a script writer.", ["Example 1: ...", "Example 2: ..."]
    )
    mock_llm.generate.assert_called_once_with(
        "Injected Prompt", "Write a script about cats."
    )
...
def test_run_without_few_shot(memory_db, tmp_path):
    ...
    assert result.status == "done"
    assert result.output_path == os.path.join(
        str(tmp_path), "data", "scripts", "job124_script.txt"
    )
    mock_llm.generate.assert_called_once_with(
        "You are a script writer.", "Write a script about cats."
    )
