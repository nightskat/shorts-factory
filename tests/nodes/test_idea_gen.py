...
def test_run_success(mock_db, tmp_path):
    ...
    with patch('shorts.nodes.idea_gen.get_llm_provider', return_value=mock_llm) as mock_get_llm:
        with patch('shorts.nodes.idea_gen.inject_voice_into_system_prompt', return_value="Injected Prompt") as mock_inject:
            result = run("job123", execution_context, mock_db, services)
...
def test_run_without_few_shot(mock_db, tmp_path):
    env = {"LLM_PROVIDER": "dummy"}
    execution_context = {
        "env": env,
        "system_prompt": "You are a script writer.",
    ...
    with patch('shorts.nodes.idea_gen.get_llm_provider', return_value=mock_llm) as mock_get_llm:
        result = run("job124", execution_context, mock_db, services)
...
def test_run_generates_error_on_missing_prompts(mock_db):
    execution_context = {"env": {}}
    services = {}
    with pytest.raises(KeyError):
        run("job125", execution_context, mock_db, services)
